...
class conterctVC: UIViewController {

    @IBOutlet weak var tableView: UITableView!
    @IBOutlet weak var statesSwitch: UISegmentedControl!
    
    var contractsBYCustomers = [GetContractsBYCustomer]()
    ...
    @objc private func handleRefresh(type: String) {
        ...
        isLoading = true
        API_countecter.GetContractsBYCustom(page: 1, customerid: "", bookingtype: type) { (error: Error?, contractsBYCustomers: [GetContractsBYCustomer]?) in
...
extension conterctVC: UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if let cell = tableView.dequeueReusableCell(withIdentifier: "cell", for: indexPath) as? conterctsCell {
            let data = contractsBYCustomers[indexPath.row]
            cell.configuerCell(prodect: data)
            cell.selectionStyle = UITableViewCell.SelectionStyle.none
            return cell
        }else {
            return conterctsCell()
        }
